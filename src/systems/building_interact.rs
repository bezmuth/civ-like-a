...
use amethyst::{
    core::{Transform, HiddenPropagate}, 
    derive::SystemDesc, ecs::WriteStorage, 
    ecs::prelude::{System, ReadStorage, ReadExpect, SystemData, Join, WriteExpect}, 
    input::{InputHandler, StringBindings}, 
    shred::Read, 
    shred::World, 
    shrev::EventChannel, shrev::ReaderId, ui::UiEvent, ui::UiFinder, ui::UiTransform, winit::MouseButton,
    };
...
impl<'s> System<'s> for BuildingInteractSystem {
    ...
    fn run(&mut self, (tiles, layer2, transforms, mouse_tile_pos, ui_finder, input, mut hidden_propagates, playersinfo, buildings): Self::SystemData) {
        ...
        if input.mouse_button_is_down(MouseButton::Left){ 
            for (tile, transform, _) in (&tiles, &transforms, &layer2).join(){
